  ...
  if (authLoading) {
    return (
      <div className="max-w-md mx-auto pt-24">
        <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100 flex items-center justify-center min-h-[300px]">
      ...
      <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-slate-100">
        ...
        {step === 0 && (
          <div className="space-y-3">
            ...
            {ROLES.map((r) => (
              <button
                key={r.value}
                onClick={() => { setRole(r.value); setError(''); setStep(1); }}
        ...
        {step === 1 && (
          <form onSubmit={handleEmailSignUp} className="space-y-4">
            <button type="button" onClick={() => { setStep(0); setError(''); }} className="flex items-center gap-2 text-sm text-slate-500 hover:text-slate-700 mb-2 transition-colors">
            ...
            <Button type="submit" disabled={submitting} className="w-full rounded-full h-12 bg-blue-600 hover:bg-blue-700 text-white font-semibold disabled:opacity-50 flex items-center justify-center gap-2">
              {submitting ? <><Loader2 className="h-5 w-5 animate-spin" /> Inscription...</> : 'S\'inscrire'}
            </Button>
